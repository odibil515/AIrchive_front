...
export default function NewsPage() {
  ...
  const handleAnalyzeNews = (id: string) => {
    ...
    setTimeout(() => {
      setNews(
        news.map((item) =>
          item.id === id ? { ...item, aiAnalyzed: true } : item
        )
      );
      ...
      toast({
        title: "News analyzed",
        description:
          "AI has analyzed this news item and updated your trading strategy",
      });
    }, 1000);
  };
  ...
  const filteredNews =
    currentTab === "all"
      ? news
      : news.filter((item) => item.category === currentTab);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-400">
              Latest news that may impact your trading strategy
            </p>
          </div>
          <Button
            variant="outline"
            ...
            <CardContent>
              <div className="p-4 bg-gray-700 rounded-md border border-gray-600">
                <p className="text-gray-300 mb-4">
                  Based on recent economic news, I've identified the following
                ...
                <ul className="space-y-2 text-gray-300">
                  <li className="flex items-start">
                    ...
                    <span>
                      The Federal Reserve's signals about potential rate cuts
                      could lead to increased liquidity in crypto markets.
                      Consider increasing exposure to major cryptocurrencies.
                    </span>
                  </li>
                  <li className="flex items-start">
                    <ThumbsUp className="h-5 w-5 text-green-400 mr-2 mt-0.5 flex-shrink-0" />
                    <span>
                      Strong tech earnings suggest a healthy market environment.
                      This positive sentiment often correlates with increased
                  ...
                  <li className="flex items-start">
                    <ThumbsUp className="h-5 w-5 text-green-400 mr-2 mt-0.5 flex-shrink-0" />
                    <span>
                      Bitcoin's increasing mining difficulty indicates network
                      strength despite price volatility. This could be a
                      positive long-term indicator.
                    </span>
                  </li>
                </ul>
              </div>
